...
func Execute(character: TowerDefenseCharacter):
    ...
    for i in floor(character.cost / everyNum):
        ...
        var projectile = FireComponent.CreateProjectilePosition(character, null, height + heightOffset, character.global_position + Vector2(randf_range(-10, 10), - heightOffset), Vector2(300.0, 0.0), "SnowPea", character.camp)
