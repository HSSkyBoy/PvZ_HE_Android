...
func AttackEntered() -> void :
    ...
    if attackAir:
        ySpeed = -200.0
        sprite.SetAnimation("BodyJump", true, 0.2 * (fireInterval + 4.5) / 6.0)
...
func AnimeEvent(command: String, argument: Variant) -> void :
    ...
    match command:
        "blow":
            var targetList = TowerDefenseManager.GetCharacterLine(gridPos.y, true)
