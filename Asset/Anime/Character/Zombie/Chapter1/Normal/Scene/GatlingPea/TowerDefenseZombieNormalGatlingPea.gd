...
func HeadIdleProcessing(delta: float) -> void :
    ...
    if global_position.x <= groundRight && fireComponent.CanFire("PeaDefault", TowerDefenseEnum.CHARACTER_COLLISION_FLAGS.GROUND_CHARACTRE):
...
func AnimeEvent(command: String, argument: Variant) -> void :
    ...
    match command:
        "fire":
            ...
            var projectile: TowerDefenseProjectile = fireComponent.CreateProjectile(0, Vector2(-300, 0), "PeaDefault", TowerDefenseEnum.CHARACTER_COLLISION_FLAGS.GROUND_CHARACTRE, camp, Vector2.ZERO)
